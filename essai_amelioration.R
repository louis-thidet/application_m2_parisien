                   ...
                   (
                     ...
                     (
                       ...
                       (
                         ...
                         # SELECTION DE LA COMMUNE A AFFICHER
                         conditionalPanel(
                           condition = "input.selection_echelle == 'commune'",
                           selectInput(
                             inputId = "commune",
                             label = "Sélection de la commune",
                             choices = NULL,
                             selected = "Paris 1er arrondissement"
                           )
                         ),
...
server <- function(input, output, session) {
  ...
  ## sert à l'application dans sa partie interface, pour connaître input$departement, et n'afficher que les communes liées au département choisi
  observe({
    if (input$selection_echelle == 'commune') {
      choices <- tabcom$nom[tabcom$dep == input$departement]
      updateSelectInput(session, "commune", choices = choices)
    }
  })
  ...
  output$histwhiskersPlot <- renderPlot({
    ...
    ## modifie le titre de la carte en conséquence
  ...
  output$mapPlot <-renderPlot({
    ...
    else if(input$selection_echelle == "commune")
    {
      tabcom<- tabcom %>% filter(nom == input$commune)
      iris <- iris %>% mutate(nom = NOM_COM)
      iris <- iris[iris$nom == input$commune, ]
      com <- com %>% mutate(nom = NOM_COM)
      com <- com[com$nom == input$commune, ]
      label_sel_echelle = input$commune
    }
    else
    {
    ...
    ## COUCHE CHOROPLETE
    if(input$selection_echelle != "commune") # Si l'échelle choisie n'est pas une seule commune, alors...
    {
      mf_map(map,
    ...
    else # si l'échelle choisie est une seule commune, alors...
    {
      # Puisque il y a une seule commune, on ne peut pas appliquer la palette, on applique donc une couleur sans rapport avec l'histogramme, pour ne pas
      # On a décidé d'inverser les couleurs par rapport à celles de l'histogramme.
      
      # Le package mapsf n'autorisant pas la carte choroplète pour un tableau d'une seule ligne, pour créer une carte pour une seule commune au rendu
      # similaire, on est obligé d'utiliser des moyens contournés. Utiliser une couche base combinée à une légende personnalisée mf_legend() n'est pas
      # possible parce que mf_legend() n'est pas compatible avec le chargement des valeurs filtrées de tabcom (Seules les valeurs de la première modalité
      # de input$commune seront chargées), donc on est contraint, pour obtenir des valeurs correctes et un affichage semblable aux autres échelles, de
      # superposer une couche prop avec une couche base.
      ...
      ## COUCHE CONTOUR DES IRIS
      if(input$couleur_legende == "Oranges")
      {
        mf_map(iris, 
               type = "base", 
               col = NA,
      ...
      else if(input$couleur_legende == "Greens")
      {
        mf_map(iris, 
               type = "base", 
               col = NA,
      ...
      else if(input$couleur_legende == "Blues")
      {
        mf_map(iris, 
               type = "base", 
               col = NA,
